...
| A submission invocation was interrupted after an external effect and returned no resumable attempt state | Pass no attempt ledger or execution ID. Invoke submission from the immutable definition/review/profile; it reads the external target, adopts an exact existing effect only after bound user confirmation, executes a new write only when absence is proved and freshly validated, and blocks on ambiguity. |
