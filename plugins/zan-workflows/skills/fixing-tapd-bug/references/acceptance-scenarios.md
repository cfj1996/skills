...
| Fresh invocation receives that complete immutable `prior_report` and only “继续” | Resume at resolver from `prior_report`; keep its retained artifact and request the resolver's missing evidence/confirmation. Do not reinterpret “继续” as authorization or advance the chain. |
| Fresh invocation receives `prior_report` plus an explicit project-selection increment | Rerun resolver with the prior artifact and new increment. When it replaces the old definition, append the old exact definition with its reason and `replaced_by` slot to `history.superseded_results`; do not invoke repair until the new definition is eligible. |
