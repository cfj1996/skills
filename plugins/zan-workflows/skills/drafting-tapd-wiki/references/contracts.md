...
- `VALIDATED` requires `effects=NONE`, one valid effective source branch, no
  unresolved critical conflict, `validation.verdict=验证通过`, and a non-empty
  `rendered_markdown`.
- `BLOCKED` requires `validation.checked_privately=true`, a non-empty blocker,
  and `rendered_markdown=null`; it does not imply any external lookup or
  write.
...
- `validation.verdict` is internal evidence. On `VALIDATED`, the user-facing
