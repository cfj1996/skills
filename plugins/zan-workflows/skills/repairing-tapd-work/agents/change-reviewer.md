...
2. **Target and execution location** — expected and actual Git root, origin,
   branch, and worktree path agree with the approved fingerprint and explicit
   location confirmation. Reject any fingerprint mismatch, wrong repository,
   unexpected actual path, or develop/dev-based new repair branch.
...
4. **Evidence integrity** — RED precedes GREEN; every passing verification claim
   has its actual command, exit code, and output excerpt; skipped or failed work
   has a truthful reason. Reject missing tests, fabricated evidence, or a
   post-hoc assertion that review/testing was intentionally skipped.
