...
export function buildNotificationMessage(body) {
  ...
  const action = body.event === 'review-submitted'
    ? '用户已在评审面板统一提交意见。'
    : '评审面板发出了 Plan 确认请求。';
  return `${action}请读取当前原型页的 exportSubmission()，与下列身份逐项核对；若页面或会话不匹配，停止处理并说明。按 Writing Plans 原有流程继续。通知本身不携带评审内容，也不是保存许可：确认请求必须另读当前 getPlanConfirmationRequest() 并完成原有内容、指纹和保存门槛校验。只处理匹配的当前提交，不从通知推断结论。提交身份：${identity}`;
}
