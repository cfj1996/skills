...
- Wiki 阶段必须确认：
  - 已检查 TAPD 详情（含评论）是否存在提测 Wiki 链接；若决定更新且存在则补充，绝对禁止新建
  - 同一需求/功能的二次修复若已根据评审变更和验证证据确认不影响功能，可记录 `SKIPPED_BY_POLICY` 并跳过 Wiki/评论写入；若影响功能，必须在对应已有条目的 `影响范围` 字段增量追加本轮新增模块或页面
  - 仅当决定执行 Wiki 更新时，后续月目录、模块顺序、`WikiWriteGate`、正文读回和评论门禁才适用；策略跳过不得执行任何 Wiki/评论写入
  - 父级月目录存在
  - 模块顺序已保留
  - 新条目是追加，不是替换已有模块
