...
通过 TAPD MCP 提取需求/Bug 详情、评论、附件、PRD、原型和图片，生成 `item-context.md`。
...
- 如果在用户确认前又出现新的附件、PRD 或补充说明，回到采集阶段补读并合并进当前 `item-context.md`
...
- 如果 `custom_field_two` 为空，才允许按提测模板兜底到 `reporter`，并在 `item-context.md` 中标注兜底来源。
