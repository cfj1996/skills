...
- 拿到 `short-id` 后，先检查项目根目录是否已有 `__test___/{short-id}/` 或 `docs/{short-id}/`；如果存在，读取历史测试、计划、验证、评审和 `docs/{short-id}/raw.md`，并据此判断是否为二次开发
- 创建或更新 `docs/{short-id}/raw.md` 作为流程总账，初始内容至少记录 TAPD id 与描述、需求/Bug 描述、流程进度和创建时间；阶段 1/2 只允许写此文件，不允许改业务代码或 TAPD 状态
