...
读取当前会话里的 TAPD 上下文、本轮范围声明和分支确认子流程结果，拆解成可执行任务，并优先衔接 Superpowers 规划工作流。
...
- 规划阶段引用已确认的分支和工作区结果，不再重新做分支二次确认
...
- 明确记录“场景判定 -> 技能选择 -> 退出条件”
- 明确记录“已确认的分支/工作区上下文如何影响当前计划”
- 任务必须具体可执行
- 首个任务优先验证根因假设
- 规划里要明确：提交完成后通过 GitLab 直接合并到 `develop`，并在合并成功后提示用户
