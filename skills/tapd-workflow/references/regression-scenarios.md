...
## 场景 K12：冲突中间分支和修复分支同时写入 Wiki（应阻断）

- 输入场景：
  - 原开发分支为 `fixbug/cfj.260722.修复积分分组删除-1016696`
  - 远端冲突后走 `A -> C -> develop`，原开发源分支为 `fixbug/cfj.260722.修复积分分组删除-1016696`，中间分支为 `merge/修复积分分组删除-to-develop-1016696`
  - Wiki 草稿写成：
    - `中间分支：merge/修复积分分组删除-to-develop-1016696`
    - `修复分支：fixbug/cfj.260722.修复积分分组删除-1016696`
    - `中间分支：merge/修复积分分组删除-to-develop-1016696`
- 期望门禁：
  - `regression-checker` 返回 `FAIL`
  - 阻断原因包含“同一个 Wiki 条目只能有一行代码分支名”
  - 正确 Wiki 字段必须只写 `- 代码分支名：fixbug/cfj.260722.修复积分分组删除-1016696`
  - `effective_wiki_branch_name` 必须等于原开发源分支；不得写入 `merge/*` 中间分支
