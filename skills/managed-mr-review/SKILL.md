...
- 子代理输出必须包含：`MR（项目名称+id）`、`结论（通过/不通过/暂缓）`、`主要问题`、`证据`、`审核使用的 HEAD SHA`、`是否可合并建议`。
